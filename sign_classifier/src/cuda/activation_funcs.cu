...
__global__ void sigmoid_kernel(const float *input, float *output, int size) {
  ...
  if (idx < size) {
    ...
    if (x >= 0) {
      ...
      output[idx] = 1.0f / (1.0f + exp_neg_x);
    } else {
      float exp_x = expf(x);
...
// Sigmoid derivative
__global__ void sigmoid_derivative_kernel(const float *input, float *output,
                                          int size) {
  int idx = blockIdx.x * blockDim.x + threadIdx.x;
  if (idx < size) {
    float s = input[idx]; // Sigmoid value
    output[idx] = s * (1.0f - s);
  }
}
