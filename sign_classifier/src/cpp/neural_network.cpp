...
void CudaNeuralNetwork::forward_internal(const std::vector<float>& X, std::vector<float>& predictions) {
    // This is your existing forward function code - just rename it
    std::cout << "=== Forward Pass Debug ===" << std::endl;
    std::cout << "Input size: " << X.size() << std::endl;
    std::cout << "Expected input size: " << input_size * batch_size << std::endl;
    ...
    if (error != cudaSuccess) {
        std::cerr << "❌ Input copy failed: " << cudaGetErrorString(error) << std::endl;
        return;
    }
    std::cout << "✅ Input copied successfully" << std::endl;
    
    // Forward pass: L1u = W1 * X + b1
    std::cout << "Computing L1u = W1 * X + b1..." << std::endl;
    std::cout << "Matrix dimensions: W1(" << hidden_size << "x" << input_size << ") * X(" << input_size << "x" << batch_size << ")" << std::endl;
    cuda_matmul(d_W1, d_X, d_L1u, hidden_size, batch_size, input_size);
    error = cudaGetLastError();
    if (error != cudaSuccess) {
        std::cerr << "❌ Matrix multiplication 1 failed: " << cudaGetErrorString(error) << std::endl;
        return;
    }
    std::cout << "✅ Matrix multiplication 1 successful" << std::endl;
    
    std::cout << "Adding bias b1..." << std::endl;
    cuda_add_bias(d_L1u, d_b1, hidden_size, batch_size);
    error = cudaGetLastError();
    if (error != cudaSuccess) {
        std::cerr << "❌ Bias addition 1 failed: " << cudaGetErrorString(error) << std::endl;
        return;
    }
    std::cout << "✅ Bias addition 1 successful" << std::endl;
    ...
    error = cudaGetLastError();
    if (error != cudaSuccess) {
        std::cerr << "❌ ReLU failed: " << cudaGetErrorString(error) << std::endl;
        return;
    }
    std::cout << "✅ ReLU successful" << std::endl;
    
    // Forward pass: L2u = W2 * L1 + b2
    std::cout << "Computing L2u = W2 * L1 + b2..." << std::endl;
    std::cout << "Matrix dimensions: W2(" << output_size << "x" << hidden_size << ") * L1(" << hidden_size << "x" << batch_size << ")" << std::endl;
    cuda_matmul(d_W2, d_L1, d_L2u, output_size, batch_size, hidden_size);
    error = cudaGetLastError();
    if (error != cudaSuccess) {
        std::cerr << "❌ Matrix multiplication 2 failed: " << cudaGetErrorString(error) << std::endl;
        return;
    }
    std::cout << "✅ Matrix multiplication 2 successful" << std::endl;
    
    std::cout << "Adding bias b2..." << std::endl;
    cuda_add_bias(d_L2u, d_b2, output_size, batch_size);
    error = cudaGetLastError();
    if (error != cudaSuccess) {
        std::cerr << "❌ Bias addition 2 failed: " << cudaGetErrorString(error) << std::endl;
        return;
    }
    std::cout << "✅ Bias addition 2 successful" << std::endl;
    ...
    std::cout << "Predictions vector resized to: " << predictions.size() << std::endl;
    ...
    if (error != cudaSuccess) {
        std::cerr << "❌ CUDA memcpy error in forward: " << cudaGetErrorString(error) << std::endl;
        predictions.clear();
        return;
    }
    std::cout << "✅ Results copied successfully" << std::endl;
    ...
    if (predictions.size() > 0) {
        std::cout << "✅ Forward pass completed. First few predictions: ";
        for (int i = 0; i < std::min(5, (int)predictions.size()); i++) {
            std::cout << predictions[i] << " ";
        }
        std::cout << std::endl;
    } else {
        std::cout << "❌ Predictions vector is empty after forward pass!" << std::endl;
    }
    
    std::cout << "=== End Forward Pass Debug ===" << std::endl;
}
